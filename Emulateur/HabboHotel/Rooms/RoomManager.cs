        ...
        public void RoomCycleTask()
        {
            if (roomCycleStopwatch.ElapsedMilliseconds >= 500)
            {
                    ...
                    foreach (Room Room in this._rooms.Values.ToList())
                    {
                        ...
                        {
                            ThreadPool.UnsafeQueueUserWorkItem(new WaitCallback(Room.ProcessRoom), null); //QueueUserWorkItem
                            Room.IsLagging = 0;
                        }
                        else
                        {
                            Room.IsLagging++;
                            if (Room.IsLagging > 10)
                            {
                                UnloadRoom(Room);
                                Console.WriteLine("L'appart avec l'id: " + Room.RoomData.Id + " est rester bloquer + de 5sec");
                            }
                        }
                    }
            }
