...
namespace Butterfly.HabboHotel.Support
{
    public class ModerationManager
    {
        ...
        public List<TicketResolution> TicketResolution1;
        public List<TicketResolution> TicketResolution2;

        public Dictionary<int, string> _moderationCFHTopics = new Dictionary<int, string>();
        ...
        public ServerPacket SerializeTool()
        {
            ...
            Response.WriteInteger(TicketResolution1.Count); //nombre de truc
            foreach (TicketResolution Resolu in TicketResolution1)
            {
                ...
                Response.WriteBoolean(true); //euh?
            }
            Response.WriteString("PII"); //catégorie titre
            Response.WriteBoolean(true);

            Response.WriteInteger(TicketResolution2.Count); //nombre de truc
            ...
            // Room tool notices
            Response.WriteInteger(RoomMessagePresets.Count);
        ...
        public void LoadMessagePresets()
        {
            ...
            using (IQueryAdapter dbClient = ButterflyEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                ...
                foreach (DataRow dataRow in table.Rows)
                {
                    ...
                    {
                        case "message":
                            this.UserMessagePresets.Add(str);
                            continue;
                        case "roommessage":
                            this.RoomMessagePresets.Add(str);
                            continue;
                        default:
                            continue;
                    }
        ...
        {
            this.TicketResolution1.Clear();
            this.TicketResolution2.Clear();
            using (IQueryAdapter dbClient = ButterflyEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                ...
                foreach (DataRow dataRow in table.Rows)
                {
                    TicketResolution str = new TicketResolution((string)dataRow["Titre"], (string)dataRow["Soustitre"], (int)dataRow["ban_hours"], (int)dataRow["enablemute"], (int)dataRow["mute_hours"], (int)dataRow["reminder"], (string)dataRow["message"]);
                    switch (dataRow["type"].ToString())
                    {
                        case "Sexual":
                            this.TicketResolution1.Add(str);
                            continue;
                        case "PII":
                            this.TicketResolution2.Add(str);
                            continue;
                        default:
                            continue;
                    }
        ...
        {
            foreach (SupportTicket supportTicket in this.Tickets)
            {
        ...
        public static void KickUser(GameClient ModSession, int UserId, string Message, bool Soft)
        {
            ...
            else
            {
                ...
                ButterflyEnvironment.GetGame().GetModerationTool().LogStaffEntry(ModSession.GetHabbo().Id, ModSession.GetHabbo().Username, 0, string.Empty, "Modtool", string.Format("Modtool kickalert: {0}", Message));

                clientByUserId.SendNotification(Message + "\r\n- " + ModSession.GetHabbo().Username);
            }
        }
        ...
        public static void AlertUser(GameClient ModSession, int UserId, string Message, bool Caution)
        {
            ...
            ButterflyEnvironment.GetGame().GetModerationTool().LogStaffEntry(ModSession.GetHabbo().Id, ModSession.GetHabbo().Username, 0, string.Empty, "ModTool", string.Format("Modtool alert ( {1} ): {0}", Message, clientByUserId.GetHabbo().Username));
